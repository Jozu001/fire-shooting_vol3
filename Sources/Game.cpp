...
// TODO: PlaySE()関数を使って、弾の発射時とターゲットに当たった時にSEを再生する。(H)   //実装:HW16A167_渕　崇洋
...
void Update()
{
    ...
    if (bulletPos.x <= -999 && Input::GetKeyDown(KeyMask::Space)) {
        PlaySound("se_maoudamashii_explosion03.mp3");       //実装:HW16A167_渕　崇洋
    ...
    if (bulletPos.x > -999) {
        ...
        if (targetRect.Overlaps(bulletRect)) {
            PlaySound("se_maoudamashii_explosion06.mp3");   //実装:HW16A167_渕　崇洋
            score += 1;         // スコアの加算
